...
test('testFlagNumber clamps to >= 0 and falls back on invalid input', () => {
  ...
  assert.equal(testFlagNumber(env, 'EXAMPLE_DURATION_MS', 60_000), 60_000)
  assert.equal(testFlagNumber({ ...env, EXAMPLE_DURATION_MS: '5000' } as NodeJS.ProcessEnv, 'EXAMPLE_DURATION_MS', 60_000), 5000)
  assert.equal(testFlagNumber({ ...env, EXAMPLE_DURATION_MS: '-5' } as NodeJS.ProcessEnv, 'EXAMPLE_DURATION_MS', 60_000), 0)
  assert.equal(testFlagNumber({ ...env, EXAMPLE_DURATION_MS: 'abc' } as NodeJS.ProcessEnv, 'EXAMPLE_DURATION_MS', 60_000), 60_000)
})

test('testFlagNumber and testFlagPercent are refused in production', () => {
  const prod = { SENTRY_ENVIRONMENT: 'production', EXAMPLE_PERCENTAGE: '100' } as NodeJS.ProcessEnv
  assert.equal(testFlagNumber(prod, 'EXAMPLE_PERCENTAGE', 0), 0)
  assert.equal(testFlagPercent(prod, 'EXAMPLE_PERCENTAGE', 0), 0)
})

test('testFlagPercent clamps to 0-100 on staging', () => {
  const env = { SENTRY_ENVIRONMENT: 'staging' } as NodeJS.ProcessEnv
  assert.equal(testFlagPercent(env, 'EXAMPLE_PERCENTAGE', 0), 0)
  assert.equal(testFlagPercent({ ...env, EXAMPLE_PERCENTAGE: '50' } as NodeJS.ProcessEnv, 'EXAMPLE_PERCENTAGE', 0), 50)
  assert.equal(testFlagPercent({ ...env, EXAMPLE_PERCENTAGE: '150' } as NodeJS.ProcessEnv, 'EXAMPLE_PERCENTAGE', 0), 100)
  assert.equal(testFlagPercent({ ...env, EXAMPLE_PERCENTAGE: '-10' } as NodeJS.ProcessEnv, 'EXAMPLE_PERCENTAGE', 0), 0)
})
