...
### 2026-08-25 - Critical Sentry runtime health for FxSocket and worker heartbeat

- **Plain English:** Sentry now has a separate critical-health path for infrastructure/runtime failures that need engineering attention. The immediate FxSocket market-data socket incident now has deterministic coverage: a brief disconnect remains a normal reconnect/log event, but a WebSocket outage that stays down beyond the configured grace period emits a critical Sentry event, suppresses retry storms inside the worker process, and uses a shared fingerprint so equivalent events group into one issue.
- **Fix (technical):** Added `worker/src/observability/criticalHealth.ts` with bounded component/failure metadata, stable fingerprints, cooldown support, hashed resource identifiers, and a `SustainedOutageTracker`. Wired `worker/src/fxsocketWsClient.ts` close/reconnect/open transitions into that tracker without changing WebSocket subscription or reconnect behavior. Extended the Sentry adapter with guarded `captureCheckIn`, added `worker/src/observability/workerHeartbeat.ts`, and starts opt-in worker heartbeat check-ins from `worker/src/index.ts` when `SENTRY_WORKER_HEARTBEAT_MONITOR_SLUG` is configured.
- **Config/docs:** Added `SENTRY_CRITICAL_HEALTH_ENABLED`, `SENTRY_CRITICAL_HEALTH_COOLDOWN_MS`, `FXSOCKET_SOCKET_OUTAGE_GRACE_MS`, `SENTRY_WORKER_HEARTBEAT_MONITOR_SLUG`, `SENTRY_WORKER_HEARTBEAT_ENABLED`, `SENTRY_WORKER_HEARTBEAT_INTERVAL_MS`, and `SENTRY_WORKER_HEARTBEAT_CHECKIN_MARGIN_MINUTES` to `worker/.env.example`. Updated Sentry and worker deployment docs to separate critical runtime health from user/trade business diagnostics.
- **Tests/verification:** New `criticalHealth.test.ts` covers short disconnect no-alert, sustained outage exactly one alert, reconnect retry dedupe, recovery reset, second later outage, Sentry disabled, and Sentry capture failure. New `workerHeartbeat.test.ts` covers opt-in slug gating, bounded monitor config, and check-in capture failure isolation. Validation: `npm run build` in `worker` passed; focused worker suite passed (40 tests); changed-file ESLint passed; `git diff --check` and conflict-marker scan passed. Root `npm run lint` still fails on pre-existing repo-wide/frontend/Supabase/worker lint debt. Worker `npm test` is still not Windows-portable because the script uses Unix `$(find ... | sort)`.
- **Deploy state:** Implemented locally on `dev`; not committed, pushed, or deployed. During the work, `origin/dev` advanced and local `dev` is now behind, so review/merge should reconcile remote changes first.
