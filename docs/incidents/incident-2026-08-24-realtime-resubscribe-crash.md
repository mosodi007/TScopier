...
**Why restarts usually masked it:** the defect lived only in the recovery path of a running process. A restart boots a fresh process with an empty Supabase client registry — no stale channel objects exist, so both subscriptions connect cleanly on first attempt and the worker resumes. Each process then ran fine until its next realtime drop, crashed, and was restarted again: production cycled healthy → drop → bad recovery → crash → clean restart roughly every 100 minutes during Aug 20–21. Restarts were an accidental safety net: users saw only brief listening gaps because `ON_FAILURE` relaunches within seconds. Staging's Aug-24 crash became visible only because the automatic relaunch did not fire (reason not determinable from the Railway API; possibly retry-budget exhaustion or a platform-side issue). Two lasting lessons: uptime figures partly reflected fast crash-restart cycling rather than true stability, and the absence of alerting on `worker-fatal` let 17 crashes pass unnoticed — alerting is added as follow-up.
