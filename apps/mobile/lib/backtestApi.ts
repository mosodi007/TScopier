...
const BACKTEST_EDGE_TIMEOUT_MS = 180_000
...
async function callBacktestEdge<T extends Record<string, unknown>>(
  ...
  let data: T
  try {
    ;({ ok, status, data } = await callEdgeFunction<T>('backtest-run', {
      accessToken: token,
      timeoutMs: BACKTEST_EDGE_TIMEOUT_MS,
      body,
    }))
  } catch {
    throw new Error(
      'Could not reach backtest-run. Deploy the edge function and apply the backtest migration.',
    )
  }
...
export async function waitForBacktestRunComplete(
  ...
  throw new Error(
    'Backtest is taking longer than expected. Open History to view the run when it completes.',
  )
}
...
export const backtestApi = {
  async sync(
    ...
    { mode: 'async'; sync_run_id: string } | { mode: 'legacy'; result: BacktestSyncResult }
  > {
    const data = await callBacktestEdge<Record<string, unknown>>({ action: 'sync', config })
    ...
    throw new Error(
      'Signal sync started but no sync run id was returned. Redeploy the backtest-run edge function.',
    )
  },
  ...
  async backtestTpsl(
    ...
    const data = await callBacktestEdge<{ ok?: boolean; run_id?: string; run_mode?: BacktestRunMode }>(
      {
