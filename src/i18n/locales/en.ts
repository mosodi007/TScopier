import { authEn } from '../auth/en'
import { channelWorkerEn } from '../channelWorker/en'
import { tradeNotificationsEn } from '../tradeNotifications/en'
import { contactSupportEn } from '../contactSupport/en'
import { riskDisclaimerEn } from '../riskDisclaimer/en'
import { termsOfServiceEn } from '../legal/termsEn'
import { privacyPolicyEn } from '../legal/privacyEn'
import { cookiePolicyEn } from '../legal/cookiePolicyEn'
import { landingEn } from './landing/en'
import type { Translations } from './types'
import { configureModalEn } from './configureModal/en'
import { statusModalEn } from './accountConfig/statusModal'

export const en: Translations = {
  auth: authEn,
  channelWorker: channelWorkerEn,
  tradeNotifications: tradeNotificationsEn,
  contactSupportPage: contactSupportEn,
  riskDisclaimerPage: riskDisclaimerEn,
  termsOfServicePage: termsOfServiceEn,
  privacyPolicyPage: privacyPolicyEn,
  cookiePolicyPage: cookiePolicyEn,
  nav: {
    sections: {
      general: 'GENERAL',
      signals: 'SIGNALS',
      discover: 'DISCOVER',
      tradingTools: 'TRADING TOOLS',
      feedback: 'FEEDBACK',
      growth: 'GROWTH & MONETIZATION',
      membership: 'MEMBERSHIP',
      help: 'HELP',
    },
    items: {
      dashboard: 'Dashboard',
      configuration: 'Configuration',
      brokers: 'Brokers',
      trades: 'Trades',
      management: 'Copier Engine',
      channels: 'Channels',
      popularChannels: 'Popular Channels',
      backtest: 'Backtest',
      copierLogs: 'Copier Logs',
      signalHistory: 'Manage Signals',
      reportedTrades: 'Reported Trades',
      marketNews: 'Market News',
      economicCalendar: 'Economic Calendar',
      performance: 'Performance',
      contactSupport: 'Contact Support',
      featureRequest: 'Feature Request',
      partnerWithUs: 'Partner with us',
      affiliateProgram: 'Affiliate program',
      billing: 'Billing',
      subscriptions: 'Subscriptions',
    },
    signOut: 'Sign out',
    search: 'Search',
    openMenu: 'Open menu',
    closeMenu: 'Close menu',
    expandSidebar: 'Expand sidebar',
    collapseSidebar: 'Collapse sidebar',
    openTradesActive: 'Open trades active',
    highImpactNewsToday: 'High impact news today',
    planFree: 'Free',
    settings: 'Settings',
    help: 'Help',
    copierPause: {
      pauseLabel: 'Pause copying',
      resumeLabel: 'Resume copying',
      pausedHint: 'Signal copying is paused',
      statusRunning: 'Copier Running',
      statusStopped: 'Resume Copier',
      statusPaused: 'Copier Paused',
      statusCopierStopped: 'Copier Stopped',
      stoppedHintSubscription: 'Subscribe to start the copier',
      stoppedHintSetup: 'Connect Telegram, add channels, and link a broker to start the copier',
      stopCopier: 'Pause Copier',
      confirmTitle: 'Pause copier',
      confirmBody: 'Signal copying, trade execution, and copier logs will stop until you resume.',
      setupBroker: 'link a broker',
      setupTelegram: 'connect Telegram',
      setupChannels: 'add a channel',
      bannerAction: 'Go to setup',
      bannerText: 'To start the copier, please {items}.',
      bannerLastSep: ' and ',
    },
    notifications: {
      title: 'Notifications',
      close: 'Close notifications',
      loading: 'Loading activity…',
      empty: 'No recent trade activity yet.',
      kindExecuted: 'Trade executed',
      kindModified: 'Trade modified',
      kindClosed: 'Trade closed',
      markAllRead: 'Mark all read',
      viewTrades: 'View trades',
      reviewPending: 'Signal waiting for your approval',
      dayToday: 'Today',
      dayYesterday: 'Yesterday',
    },
    userMenu: {
      menuLabel: 'Account menu',
      profileSettings: 'Profile & Settings',
      subscriptionBilling: 'Subscription & Billing',
      affiliateProgram: 'Affiliate Program',
      rateUs: 'Rate us',
      signOut: 'Sign Out',
    },
    helpMenu: {
      title: 'Help & support',
      documentation: 'Documentation',
      liveChat: 'Help & Support',
      status: 'Status',
      statusOperational: 'All systems operational',
    },
    assistant: {
      title: 'TScopier Assistant',
      subtitle: 'Ask questions or get help taking action',
      ariaLabel: 'Open AI assistant',
      close: 'Close assistant',
      clearConversation: 'Clear conversation',
      historyTitle: 'Chat history',
      newChat: 'New chat',
      historyEmpty: 'No past conversations yet.',
      deleteChat: 'Delete conversation',
      placeholder: 'Ask anything about TScopier…',
      send: 'Send',
      thinking: 'Thinking…',
      confirm: 'Confirm',
      cancel: 'Cancel',
      welcomeTitle: 'I can help you set up and run TScopier.',
      suggestions: [
        'Is my Telegram linked?',
        'Connect my MT5 account',
        'Configure broker 928883 with lot 0.02',
        'Pause the copier',
      ],
      emptyReply: 'How else can I help?',
      actionDone: 'Done.',
      errorFallback: 'Something went wrong. Please try again.',
      attachImage: 'Attach image',
      removeImage: 'Remove image',
      imageOnlyCaption: 'Please look at this image.',
      imageTooLarge: 'Image is too large. Try a smaller screenshot.',
      imageTypeUnsupported: 'Use a JPEG, PNG, WebP, or GIF image.',
      imageLimitReached: 'You can attach up to 3 images per message.',
      telegram: {
        phoneTitle: 'Link Telegram',
        phoneHint: 'Enter your Telegram phone number in international format.',
        phonePlaceholder: '+1…',
        sendCode: 'Send code',
        codeTitle: 'Enter verification code',
        codeHint: 'Telegram sent a login code to your app or SMS. Enter it here — not in the chat box.',
        codePlaceholder: '12345',
        verifyCode: 'Verify',
        twoFaTitle: 'Two-step verification',
        twoFaHint: 'This Telegram account has a cloud password. Enter it below — never in the chat box.',
        twoFaPlaceholder: 'Cloud password',
        submitPassword: 'Continue',
        restart: 'Start over',
        openQrInstead: 'Prefer QR code? Open Copier Engine',
        invalidPhone: 'Enter a valid phone number with country code (e.g. +1…).',
        codeSent: 'Great — a code has been sent to Telegram. Enter it in the secure field below to continue.',
        twoFaNeeded: 'Telegram needs your two-step verification password. Enter it in the secure field below.',
        linkedSuccess: 'Telegram is linked. You can add channels next on Copier Engine.',
        useSecureCodeField: 'Enter the code in the secure field above — don’t send it in chat.',
        sessionExpired: 'Login session expired. Request a new verification code.',
        composerPhoneHint: 'Type your number here or use the field above…',
        composerCodeHint: 'Use the secure code field above…',
      },
      tradesCard: {
        title: 'Trades',
        noTrades: 'No recent trades found.',
        ticket: 'Ticket',
        legs: 'Legs',
        viewDetails: 'View details',
        statusExecuted: 'Executed',
        statusFailed: 'Failed',
        statusSkipped: 'Skipped',
        statusPending: 'Pending',
        statusParsed: 'Parsed',
        statusDispatched: 'Dispatched',
        statusIgnored: 'Ignored',
        statusError: 'Error',
        statusCancelled: 'Cancelled',
        positionOpen: 'Open',
        positionClosed: 'Closed',
        reportStatusOpen: 'Open',
        reportStatusResolved: 'Resolved',
      },
broker: {
        title: 'Connect trading account',
        hint: 'Enter your broker details. The password stays in this secure card — never type it in chat.',
        platformLabel: 'Platform',
        loginLabel: 'Account login',
        serverLabel: 'Broker server',
        labelLabel: 'Label (optional)',
        passwordLabel: 'Password',
        passwordPlaceholder: 'Investor or trading password',
        connect: 'Connect',
        openFullForm: 'Open full connect form (server picker)',
        missingFields: 'Login, server, and password are required.',
        connectedSuccess: 'Broker connected. Tell me which channel to configure and your lot size when you’re ready.',
      },
    },
  },
  globalSearch: {
    placeholder: 'Search pages, accounts, channels…',
    noResults: 'No results found',
    groupPages: 'Pages',
    groupBrokers: 'Broker accounts',
    groupChannels: 'Signal channels',
    shortcut: '⌘K',
  },
  pricing: {
    title: 'Choose your plan',
    subtitle: 'Start copying signals to your trading accounts today.',
    skip: 'Skip for now',
    monthly: 'Monthly',
    annual: 'Annual',
    perMonth: '/month',
    perYear: '/year',
    save20: 'Save 20%',
    billedAnnually: 'Billed annually',
    extraAccountLabel: 'Additional accounts',
    extraAccountUnit: '$10/account/month',
    extraAccountUnitAnnual: '$96/account/year',
    subscribe: 'Subscribe',
    startTrial: 'Start your 5-day free trial',
    trialDays: '5-day free trial on Advanced',
    moneyBackGuarantee: '5-day free trial on Advanced',
    features: 'Features',
    popular: 'Most Popular',
    basicFeatures: [
      '1 demo/live account',
      '5 Signal Backtests/month',
      '5 Telegram Channels',
      '3 TPs',
      'Single Trading Mode',
      'Market News',
      'Economic Calendar',
      'Time/Days/News Filter',
    ],
    advancedFeatures: [
      '5 demo/live accounts (expandable to 100)',
      'Unlimited signal backtests/month',
      'Unlimited Telegram Channels',
      'Unlimited TPs/SLs',
      'Single & Range Trading Mode',
      'Range Layering',
      'Close worse entries first',
      'Reverse signal',
      'Auto breakeven & close (Pips, Money, RR, TP Hit)',
      'Risk Reward Mode',
      'Market News',
      'Economic Calendar',
      'Time/Days/News Filter',
      'Channel Keyword follow',
    ],
    extraAccountsSummary:
      '{included} included + {extra} extra = {total} total accounts',
    basic: {
      name: 'Basic',
      description: 'For beginner traders looking for a simple Telegram signal copier for their MT4/5.',
    },
    advanced: {
      name: 'Advanced',
      description: 'For serious traders copying signals to 1 or multiple accounts with advanced strategies.',
    },
    custom: {
      name: 'Custom',
      description:
        'For prop firms, teams, and high-volume traders who need tailored account limits and dedicated support.',
    },
    contactUs: 'Contact us',
    contactSales: 'Contact sales',
    customFeatures: [
      'Custom account limits',
      'Dedicated onboarding',
      'Priority support',
      'Tailored billing & invoicing',
      'All Advanced copier features',
      'Volume & team pricing',
    ],
    billing: {
      title: 'Subscription & Billing',
      subtitle: 'Manage your plan, payment methods, and invoice history.',
      customerId: 'Customer ID',
      credit: 'Credit',
      purchaseSubscription: 'Purchase Subscription',
      memberSince: 'Member since',
      nextRenewal: 'Next renewal',
      currentPlan: 'Current Plan',
      billed: 'Billed',
      noActiveSubscription: 'No active subscription',
      noSubscriptionHeaderBanner: 'You have no active subscription.',
      freePlan: 'Free',
      freePlanSummary: 'Browse only — subscribe to connect brokers & copy signals',
      basicPlanSummary: '1 broker · 5 channels · 3 TPs · Single trade mode',
      advancedPlanSummary: '5+ accounts · Unlimited channels · Advanced copier tools',
      adminPlanSummary: 'Unlimited accounts · Unlimited channels · Advanced copier tools',
      adminPlanSummaryUntil: 'Admin access until {date} · Unlimited accounts · Unlimited channels · Advanced copier tools',
      statusLine: 'Status: {status}',
      invoices: 'Invoices',
      invoicesHint: 'Download PDF copies of your past invoices.',
      invoiceNumber: 'Invoice #',
      period: 'Period',
      date: 'Date',
      amount: 'Amount',
      status: 'Status',
      downloadInvoice: 'Download Invoice',
      invoiceStatusPaid: 'Paid',
      invoiceStatusDraft: 'Draft',
      invoiceStatusOpen: 'Open',
      invoiceStatusVoid: 'Void',
      invoiceStatusUncollectible: 'Uncollectible',
      noInvoices: 'No invoices yet.',
      loadInvoicesFailed: 'Could not load billing history.',
      subscriptionPlans: 'Subscription Plans',
      subscriptionPlansIntro: 'Compare plans and billing cycles — annual billing saves 20%.',
      pricingTablePlans: 'Plans',
      pricingTableIncludes: 'Includes',
      pricingTableMonthly: 'Monthly',
      pricingTableAnnual: 'Annual',
      pricingTableAction: 'Action',
      pricingTableCategory: 'Copier',
      pricingTableBestValue: 'Best value',
      pricingTablePurchase: 'Purchase',
      pricingTableSave: 'Save {amount}',
      pricingTableAdvancedIncludes: '{count} demo/live accounts (expandable to 100)',
      pricingTableHint: 'Click a monthly or annual price to select billing cycle, then Purchase.',
      back: 'Back',
      next: 'Next',
      nextBilling: 'Next billing date',
      extraAccounts: 'Extra accounts',
      extraAccountsSummary: '{included} included + {extra} extra = {total} total accounts',
      manageBilling: 'Manage billing',
      noPlan: 'No active subscription',
      statusActive: 'Active',
      statusTrialing: 'Trial',
      statusCanceled: 'Canceled',
      statusPastDue: 'Past due',
      trialEnds: 'Trial ends',
      choosePlan: 'Start your 5-day free trial',
    },
    checkoutFailed: 'Could not start checkout. Please try again.',
    paywall: {
      noPlanTitle: 'Subscribe to start copying signals',
      noPlanReason: "You're on view only mode. Purchase a subscription plan to connect brokers, Telegram channels and backtests",
      upgradeTitle: 'Upgrade required',
      upgradeCta: 'Upgrade to Advanced',
      purchaseSubscriptionCta: 'Purchase Subscription',
      manageBilling: 'Manage billing',
      updatePaymentTitle: 'Payment did not go through',
      updatePaymentReason:
        'Please pay your invoices to avoid service disruptions. We will stop the copier if payment is not received.',
      updatePayment: 'Pay invoices',
      brokerLimit: 'Your plan allows {limit} broker account(s). Upgrade to connect more.',
      channelLimit: 'Basic plan includes {limit} Telegram channels. Upgrade for unlimited channels.',
      backtestLimit: 'Basic plan includes {limit} backtests per month. Upgrade for unlimited backtests.',
      subscriptionRequired: 'An active subscription is required for this action.',
      advancedFeature: 'This setting is available on the Advanced plan.',
    },
  },
  landing: landingEn,
  common: {
    comingSoon: 'Coming soon',
    underDevelopment: 'This feature is under development',
    loading: 'Loading…',
    refresh: 'Refresh',
    save: 'Save',
    cancel: 'Cancel',
    delete: 'Delete',
    edit: 'Edit',
    add: 'Add',
    all: 'All',
    yes: 'Yes',
    no: 'No',
    preview: 'Preview',
    synced: 'synced {time}',
    won: 'Won {count}',
    lost: 'Lost {count}',
    breakeven: '{count} breakeven',
    vsYesterday: '{amount} vs yesterday',
    perPage: 'Per page',
    previous: 'Previous',
    next: 'Next',
    showingRange: '{start}–{end} of {total}',
    noResults: 'No results',
    show: 'Show',
    results: 'results',
    cookieConsent: {
      message: 'We use cookies to improve user experience.',
      policyLink: 'Cookie Policy',
      accept: 'Accept cookies',
      dismiss: 'Dismiss',
    },
  },
  accountConfig: {
    brokersEmptyTitle: 'No accounts connected yet',
    brokersEmptySubtitle: 'Add your trading account to get started',
    addAccount: {
      title: 'Add a trading account',
      subtitle: 'Choose your preferred trading platform to get started',
      footerHint: 'More platforms coming soon',
      comingSoonBadge: 'Coming Soon',
      comingSoonPlatform: '{platform} integration is coming soon. Pick MT4 or MT5 for now.',
    },
    connectForm: {
      addAccountButton: 'Add account',
      title: 'Connect a new {platform} account',
      accountLabel: 'Account label (optional)',
      accountLabelPlaceholder: 'e.g. Live {platform}',
      platformLabel: 'Platform',
      platformMt5: 'MetaTrader 5 (MT5)',
      platformMt4: 'MetaTrader 4 (MT4)',
      brokerServerLabel: 'Broker server',
      brokerServerHint:
        'Paste the exact server name from your MetaTrader terminal (File → Login to Trade Account).',
      brokerServerPlaceholder: 'e.g. ICMarketsSC-MT5',
      brokerCompanySearchPlaceholder: 'Search by broker company or server name',
      brokerCompanySearchServersHeading: 'Servers',
      brokerCompanySearchCompaniesHeading: 'Brokers',
      brokerCompanySearchEmpty: 'Search for your broker company or server name',
      brokerCompanySearchMinChars: 'Type at least 4 characters to search',
      brokerCompanySearchNoResults: 'No matches in our broker directory.',
      brokerCompanySearchUseQuery: 'Use "{query}" as server name',
      brokerCompanySearchLoading: 'Searching brokers…',
      brokerCompanySearchError: 'Broker search failed. Try again or enter your server manually.',
      brokerServerPickerTitle: 'Server',
      brokerServerSelectPrompt: 'Search for your broker company',
      brokerServerManualToggle: "Can't find your broker? Enter server manually",
      brokerServerManualLabel: 'Server name',
      brokerServerManualHint: 'Use the exact server name from your MT terminal.',
      mtLoginLabel: 'MT login',
      mtLoginPlaceholder: 'Trading account number',
      passwordLabel: 'Password',
      passwordPlaceholder: 'Trading account password',
      passwordHint: '',
      rememberPasswordLabel: 'Remember password for automatic reconnect',
      rememberPasswordHint:
        'Encrypts your MT password on our servers so TScopier can restore the session without prompting you. You can remove it anytime.',
      connectButton: 'Connect account',
      connectingTitle: 'Connecting your broker',
      connectingStepLinking: 'Linking your {platform} account…',
      connectingStepTerminal: 'Starting your {platform} terminal — this usually takes 10–30 seconds.',
      connectingStepSlow: 'Still working… first-time setup can take a few minutes.',
      validationRequired: 'Account number, password, and server are required',
      connectFailed: 'Failed to connect account',
      addMoreButton: 'Add more',
      removeRowAria: 'Remove account row',
      connectMultipleButton: 'Connect {count} accounts',
      uploadAccountsButton: 'Upload accounts',
      accountRowTitle: 'Account {index}',
    },
    bulkConnect: {
      title: 'Upload MT4/MT5 accounts',
      securityNote:
        'Your CSV contains trading passwords. It is parsed in your browser only and is never uploaded as a file.',
      downloadTemplate: 'Download template CSV',
      uploadCsv: 'Upload CSV',
      uploadHint: 'Drop a CSV file here or click to browse. Use the platform column (MT4 or MT5) per row — defaults to MT5 if omitted.',
      previewTitle: 'Preview',
      colLabel: 'Label',
      colPlatform: 'Platform',
      colServer: 'Server',
      colLogin: 'Login',
      colPassword: 'Password',
      colStatus: 'Status',
      parseErrorLine: 'Line {line}: {message}',
      noValidRows: 'No valid accounts found in the CSV.',
      connectCount: 'Connect {count} accounts',
      connectingTitle: 'Linking accounts…',
      statusQueued: 'Queued',
      statusLinking: 'Linking…',
      statusLinked: 'Linked',
      statusFailed: 'Failed',
      statusSkippedDuplicate: 'Duplicate',
      statusSkippedLimit: 'Limit reached',
      statusSkippedInvalid: 'Invalid',
      summaryTitle: 'Accounts linked',
      summaryBody: '{linked} linked, {failed} failed, {skipped} skipped.',
      summaryFailedTitle: 'Failed accounts',
      dismiss: 'Close',
      viewBrokers: 'View brokers',
    },
    brokerList: {
      statusPaused: 'Paused',
      statusConnected: 'Connected',
      statusConnecting: 'Connecting',
      statusRecovering: 'Reconnecting',
      statusError: 'Error',
      statusDisconnected: 'Disconnected',
      statusHealthy: 'Healthy',
      statusUnhealthy: 'Unhealthy',
      statusHealthChecking: 'Checking…',
      statusHealthView: 'View broker health status',
      copyTrades: 'Copy trades',
      reconnect: 'Reconnect',
      reconnectAll: 'Reconnect all',
      configure: 'Configure',
      removeAria: 'Remove {label}',
      detailLogin: 'Login',
      detailAccountType: 'Account type',
      accountTypeDemo: 'Demo',
      accountTypeLive: 'Live',
      accountTypePropFirm: 'Prop Firm',
      detailServer: 'Server',
      detailSignalChannels: 'Signal channels',
      detailBalance: 'Balance',
      detailEquity: 'Equity',
      channelsNoneSelected: 'None selected',
      channelsEmptySaveWarning:
        'No signal channels selected — this broker will not copy any Telegram signals. Save anyway?',
      channelsSaveChannelListNotReady:
        'Channel list is still loading. Wait a moment and try saving again.',
      channelsSaveLinkedChannelsInvalid:
        'Linked signal channels could not be saved. Refresh the page and try again.',
      channelsSignalChannel: 'Signal channel',
      channelsAll: 'All signal channels',
      relinkOne:
        'This account uses an older link format. Remove it and connect again with your MT login and password.',
      relinkMany:
        '{count} accounts use an older link format. Remove each one and connect again with your MT login and password.',
  reconnectDroppedOne:
    'Trading session expired on the trade server. Use Reconnect and enter your current MT password.',
  reconnectDroppedMany:
    '{count} accounts lost their broker connection and are shown as Disconnected. Use Reconnect on each account.',
  connectErrorWrongPassword:
    'The MT account password is incorrect. Check the password in your MetaTrader terminal, then try again.',
  connectErrorWrongLogin:
    'The MT login number does not match this broker server. Verify the account number from MetaTrader.',
  connectErrorWrongServer:
    'The broker server name is incorrect or does not match this login. Check the exact server name from MetaTrader.',
  connectErrorInvestorPassword:
    'An investor (read-only) password was used. Connect with the main trading password from MetaTrader.',
  connectErrorAccountDisabled:
    'This MT account is disabled or blocked at the broker. Contact your broker or log in via MetaTrader first.',
  connectErrorCredentialsRejected:
    'Could not log in with these MT details. Verify your account number, trading password, and exact server name from MetaTrader.',
  connectErrorTerminalNotReady:
    'We could not load your account from the broker yet. If you just connected, wait a minute and try again. Otherwise verify your MT login, password, and server name match MetaTrader exactly.',
  connectErrorSessionExpired:
    'Trading session expired on the trade server. Use Reconnect and enter your current MT password.',
  connectErrorThrottled:
    'Too many connection requests in a short time. Please wait about a minute and try again.',
  connectErrorUnknown:
    'Broker connection failed. Check your MT login details or use Reconnect if this account was linked before.',
  reconnectFailed: 'Could not reconnect broker',
      reconnectPasswordTitle: 'Broker session expired',
      reconnectPasswordBody:
        'Your broker session expired on the trade server. Enter your MT account password to reconnect.',
      reconnectPasswordLabel: 'MT account password',
      reconnectPasswordHint: 'Sent to MT servers only. Enable remember below to store it encrypted for automatic reconnect.',
      reconnectPasswordPlaceholder: 'Trading account password',
      rememberPasswordLabel: 'Remember password for automatic reconnect',
      rememberPasswordHint:
        'Stores an encrypted copy so TScopier can reconnect without asking again. You can clear it from Account Configuration.',
      clearStoredCredentials: 'Forget saved password',
      storedCredentialsActive: 'Auto-reconnect enabled',
      deleteFailed: 'Failed to delete broker',
      deleteSessionExpired:
        'Your sign-in session expired. Refresh the page and try again, or sign out and back in.',
      duplicateMtLogin:
        'This MT login is already linked on another account here. Remove it first or use Reconnect — the same login cannot be connected twice.',
      platformServerMismatchMt4:
        'This server name looks like MT4, but you selected MT5. Copying and trade management may fail. Connect as MT4 instead?',
      platformServerMismatchMt5:
        'This server name looks like MT5, but you selected MT4. Copying and trade management may fail. Connect as MT5 instead?',
      deleteTitle: 'Remove trading account?',
      deleteBody:
        'This disconnects {label} from your broker and the copier. This cannot be undone.',
      deleteConfirm: 'Disconnect',
      connectedAccountsHeading: 'Connected accounts',
      connectedAccountsUnlimited: 'Unlimited',
      brokerFilterLabel: 'Broker',
      brokerFilterAll: 'All brokers',
      brokerFilterNoMatch: 'No accounts match this broker.',
      accountSearchLabel: 'Search accounts',
      accountSearchPlaceholder: 'Label, login, server, broker…',
      accountSearchNoMatch: 'No accounts match your search.',
    },
    brokerConnectedSuccess: {
      title: 'Broker connected',
      titlePending: 'Broker linked',
      body: '{account} is connected and ready to copy signals.',
      bodyPending:
        '{account} is linked. Your MT5 terminal is starting — you can configure channels while it connects.',
      addChannel: 'Add Channel',
      configure: 'Configure',
    },
    configureModal: configureModalEn,
    statusModal: statusModalEn,
  },
  dashboard: {
    title: 'Dashboard',
    loadingMetrics: 'Loading your dashboard metrics',
    totalBalance: 'Total Balance',
    acrossAccounts: 'Across {count} connected account(s)',
    todaysProfit: "Today's Profit",
    todaysProfitHint:
      'Realized P/L from copier-attributed trades closed today, since your broker connected.',
    tradesTakenToday: 'Trades Completed Today',
    tradesVsYesterday: '{count} vs yesterday',
    noClosedTradesToday: 'No closed trades today',
    openPnl: 'Open PnL',
    openPnlNoOpen: 'No open PnL',
    openPnlAcrossOneAccount: 'From 1 account',
    openPnlAcrossAccounts: 'From {count} accounts',
    openPnlModalTitle: 'Open signal positions',
    openPnlModalSubtitle: '{count} accounts · {pnl} open P/L',
    openPnlModalEmpty: 'No open signal-channel positions across your accounts.',
    closeAllAccounts: 'Close all accounts',
    closeAllAccountsConfirm: 'Close all open signal positions on every linked account?',
    activeSignalChannels: 'Active Signal Channels',
    connectedTelegramChannels: 'Connected Telegram channels',
    manageChannels: 'Manage channels',
    openTrades: 'Open Trades',
    activeBrokerPositions: 'Active broker positions',
    tradingAccountsConnected: 'Trading Accounts Connected',
    addOrManageAccounts: 'Add or manage accounts',
    tradesCopiedToday: 'Trades Copied Today',
    executedFromSignals: 'Executed from channel signals',
    aiExpertLog: 'AI Expert Log',
    copierLogs: 'Copier Logs',
    viewAll: 'View all',
    noLogsYet: 'No activity yet',
    healthStable: 'Stable',
    healthDegraded: 'Degraded',
    healthOffline: 'Offline',
    copierStatus: {
      title: 'Copier Status',
      allBrokerConnections: 'All broker connections',
      healthy: 'Healthy',
      issues: 'Issues',
      none: 'No accounts',
      copierEngine: 'Copier engine',
      live: 'Live',
      offline: 'Offline',
      checking: 'Checking…',
      telegramListener: 'Telegram Listener',
      online: 'Online',
      brokerErrors: 'Broker Errors',
      allChecksPassed: 'All check passed.',
      checksFailed: 'Issues detected.',
    },
    tradeOutcomeTitle: 'Trade Outcome (7 days)',
    tradeOutcomeSubtitle: 'Daily P/L from copier trades since your broker connected',
    tradeOutcomeEmpty: 'No copier-attributed closed trades in the last 7 days',
    chartProfit: 'Profit',
    chartLoss: 'Loss',
    accountGrowthTitle: 'Account growth (7 days)',
    accountGrowthSubtitle:
      'Account balance over the last 7 days (current balance with closed-trade P/L applied day by day)',
    accountGrowthEmpty: 'Connect a trading account to see balance growth',
    channelProfitTitle: 'Profit by signal channel (7 days)',
    channelProfitSubtitle: 'Copier trade P/L by Telegram channel since your broker connected',
    channelProfitEmpty: 'No copier-attributed closed trades in the last 7 days',
    tradeActivities: 'Copier Engine',
    tradeActivitiesHint:
      'Live trade actions from your signal channels — orders, stop-loss updates, breakeven moves, and closes. Retry failed actions from Copier Engine.',
    noTradeActivities: 'No copier engine activity yet.',
    management: 'View all',
    noData: 'No Data',
    telegramNotConnectedTitle: 'Telegram not connected',
    telegramNotConnectedBody: 'Connect your Telegram account to start copying signals from your channels.',
    connectTelegram: 'Connect Telegram',
    pastDuePaymentTitle: 'Payment did not go through',
    pastDuePaymentBody:
      'Please pay your invoices to avoid service disruptions. We will stop the copier if payment is not received.',
    payInvoices: 'Pay invoices',
    linkedAccounts: {
      title: 'Linked Accounts',
      subtitle: 'Connected broker accounts used by the copier',
      empty: 'No linked accounts yet.',
      colAccount: 'Account',
      colBroker: 'Broker',
      colAccountType: 'Account type',
      colBalance: 'Balance',
      colPnl: 'PnL',
      colPnlHint: 'Current balance minus initial balance since connect, excluding deposits and withdrawals.',
      colOpenPnl: 'Open PnL',
      colOpenPnlHint: 'Unrealized profit/loss on open positions from the broker',
      colWinRate: 'Win rate',
      colDd: 'DD',
      colStatus: 'Status',
      unnamedAccount: 'Unnamed account',
      statusActive: 'Active',
      statusPaused: 'Paused',
      statusConnected: 'Connected',
      statusConnecting: 'Connecting',
      statusRecovering: 'Reconnecting',
      accountTypeLive: 'Live',
      accountTypeDemo: 'Demo',
      accountTypePropFirm: 'Prop Firm',
      statusDisconnected: 'Disconnected',
      reconnect: 'Reconnect',
    },
    brokerStats: {
      close: 'Close',
      refresh: 'Refresh',
      notFound: 'Broker account not found.',
      initialBalance: 'Initial balance',
      initialBalanceHint: 'Account balance snapshotted when this broker first connected to TScopier.',
      connectedAt: 'Connected',
      connectedAtHint: 'Date and time this broker account was first linked and ready to trade.',
      currentBalance: 'Current balance',
      currentEquity: 'Current equity',
      totalProfit: 'Total profit / loss',
      totalProfitHint: 'Current balance minus initial balance, excluding deposits and withdrawals.',
      todayProfit: 'Profit today',
      todayProfitHint: 'Realized P/L from closed trades today (local calendar day).',
      winRate: 'Win rate',
      maxDrawdown: 'Max drawdown',
      roi: 'ROI',
      closedDeals: 'Closed deals',
      connectedChannels: 'Connected signal channels',
      connectedChannelsHint:
        'Lifetime P/L from trades copied from each connected Telegram channel (realized closed deals plus open positions).',
      noChannels: 'No attributed signal-channel trades yet.',
      tradesCount: '{count} trades',
      activeSignalTrade: 'Active signal trade',
      activeSignalTradeHint: 'Open positions per Telegram signal channel (total lots and unrealized P/L).',
      noActiveSignalTrade: 'No open signal-channel positions.',
      openPnl: 'Open P/L',
      openedAt: 'Opened',
      direction: 'Direction',
      lots: 'Lots',
      lastSignalTrade: 'Last signal trade',
      lastSignalTradeHint:
        'Most recent closed trade from a Telegram signal, with total P/L for that signal (all copies and TPs).',
      noLastSignalTrade: 'No attributed signal-channel trades in history.',
      channel: 'Channel',
      symbol: 'Symbol',
      lastSignalTotalProfit: 'Signal total P/L',
      seeAll: 'See all',
      profit: 'Profit',
      closedAt: 'Closed',
      closeChannel: 'Close',
      closeAllChannels: 'Close all',
      closeChannelConfirm: 'Close all open positions for {channel} on this account?',
      closeAllConfirm: 'Close all open signal positions on this account?',
      closeConfirmTitleChannel: 'Close channel positions?',
      closeConfirmTitleBroker: 'Close all signal positions?',
      closeConfirmTitleAllAccounts: 'Close all accounts?',
      closeConfirmBodyChannel: 'All open positions for {channel} on this account will be closed at market.',
      closeConfirmBodyBroker: 'Every open signal-channel position on this account will be closed at market.',
      closeConfirmBodyAllAccounts: 'Every open signal-channel position across {count} linked accounts will be closed at market.',
      closeConfirmAction: 'Close positions',
      closeConfirmWarning: 'This cannot be undone. Positions are closed at market price.',
      closeConfirmAccount: 'Account',
      closeConfirmPositionsLabel: 'Positions',
      closeConfirmPositions: '{count} positions',
      closeConfirmLots: '{lots} lots',
      closeConfirmChannelCount: '{count} channels',
      closing: 'Closing…',
      closeSuccess: 'Closed {count} position(s).',
      closePartial: 'Closed {closed} of {total}. Some positions could not be closed.',
      closeFailed: 'Could not close positions.',
      closeDisconnected: 'Connect this account before closing trades.',
    },
  },
  management: {
    title: 'Copier Engine',
    subtitle: 'Review copier engine activity from your signal channels.',
    filterAll: 'All',
    filterSuccessful: 'Successful',
    filterSkipped: 'Skipped',
    filterFailed: 'Failed',
    statusSuccessful: 'Successful',
    statusSkipped: 'Skipped',
    statusFailed: 'Failed',
    retry: 'Retry',
    retryAll: 'Retry All',
    retrySuccess: 'Retry sent — a new activity will appear shortly.',
    retryAllSuccess: 'Retried {count} failed activities.',
    retryFailedGeneric: 'Could not retry this activity.',
    retryFailedDetail: 'Could not retry: {reason}',
    retryReasonNotFound: 'Activity not found.',
    retryReasonNotFailed: 'Only failed activities can be retried.',
    retryReasonNotEligible: 'This activity type cannot be retried.',
    retryReasonNoSignal: 'The linked signal is missing.',
    retryReasonNotRetryable: 'The signal cannot be run again.',
    retryReasonDispatchRejected: 'Copier could not accept the retry.',
    retryReasonWorkerUnavailable: 'Trade worker is unavailable — try again shortly.',
    emptyTitle: 'No activities yet',
    emptySubtitle: 'Copier engine activity will appear here once your copier executes actions from signal channels.',
    emptyFailedTitle: 'No failed activities',
    kindBreakeven: 'SL to Breakeven',
    kindClose: 'Close trade',
    kindCloseWorseEntries: 'Close worse entries',
    kindModify: 'Update SL/TP',
    kindOrder: 'Order placement',
    kindLayering: 'Layered entry',
    kindPipeline: 'Signal processing',
    kindOther: 'Trade activity',
  },
  copierLogs: {
    title: 'Copier Logs',
    subtitle: 'Full history of signals received and their execution outcome',
    filterAll: 'All',
    filterExecuted: 'Executed',
    filterSkipped: 'Skipped',
    filterFailed: 'Failed',
    filterPending: 'Pending',
    colStatus: 'Status',
    colReason: 'Reason',
    colChannel: 'Channel',
    colSymbol: 'Symbol',
    colMessage: 'Message',
    colType: 'Type',
    colTime: 'Time',
    emptyTitle: 'No logs yet',
    emptySubtitle: 'Signal logs will appear here once your copier receives messages',
    statusExecuted: 'Executed',
    statusSkipped: 'Skipped',
    statusFailed: 'Failed',
    statusPending: 'Pending',
    statusParsed: 'Parsed',
    retry: 'Retry',
    retrySuccess: 'Retry sent — the copier will attempt this signal again.',
    retryFailedGeneric: 'Could not retry this signal.',
    retryFailedDetail: 'Could not retry: {reason}',
    retryReasonNotRetryable: 'This signal cannot be retried.',
    retryReasonDispatchRejected: 'The copier did not accept the retry.',
    skipReasons: {
      invalid_stops: 'Broker rejected stop levels',
      entry_not_opened: 'No position opened',
      entry_zone_far_from_market: 'Entry too far from market',
      broker_session_not_connected: 'Broker not connected',
      broker_bridge_unavailable: 'Broker bridge unavailable',
      broker_reactivated_after_signal: 'Signal while broker was offline',
      channel_max_risk_hit: 'Daily risk limit reached',
      channel_profit_target_hit: 'Daily profit target reached',
      channel_config_missing: 'Channel not configured',
      channel_config_incomplete: 'Channel settings incomplete',
      channel_filter_ignored: 'Ignored by channel filter',
      no_broker_channel_match: 'No broker linked to channel',
      copier_paused: 'Copier is paused',
      telegram_listener_not_live: 'Telegram not connected',
      subscription_inactive: 'Subscription inactive',
      plan_advanced_feature_required: 'Plan upgrade required',
      entry_not_execution_eligible: 'Not eligible to trade',
      duplicate_provider_signal: 'Duplicate signal',
      explicit_stops_required_when_add_to_existing_off: 'SL/TP required',
      entry_tp_without_sl: 'SL not given — set predefined SL pips in broker configuration',
      signal_missing_required_sl: 'SL not given — set predefined SL pips in broker configuration',
      basket_modify_failed: 'Could not update open trades',
      parameter_follow_up_no_open_basket: 'No open trade to update',
      mgmt_no_open_trades: 'No matching open trade',
      mgmt_no_open_trades_db: 'No open trade in copier',
      mgmt_no_open_trades_broker: 'No open position on broker',
      mgmt_no_open_trades_symbol: 'Symbol did not match',
      no_matching_open_trade: 'No matching open trade',
      symbol_not_in_whitelist: 'Symbol not allowed',
      symbol_excluded: 'Symbol excluded',
      symbol_exempted_from_trading: 'Symbol exempted',
      close_worse_entries_disabled: 'Close worse entries disabled',
    },
    detailModal: {
      title: 'Signal details',
      close: 'Close',
      status: 'Status',
      reason: 'Reason',
      whatHappened: 'What happened',
      channel: 'Channel',
      symbol: 'Symbol',
      action: 'Action',
      levels: 'Parsed levels',
      entry: 'Entry',
      stopLoss: 'Stop loss',
      takeProfit: 'Take profit',
      signalMessage: 'Telegram message',
      technicalCode: 'Technical code',
      timeline: 'Execution timeline',
      loadingTimeline: 'Loading execution details…',
      noTimeline: 'No additional execution events were recorded.',
      viewDetails: 'View details',
      receivedAt: 'Received',
    },
  },
  signalHistoryPage: {
    allChannels: 'All channels',
    dateFrom: 'From',
    dateTo: 'To',
    presetAll: 'All time',
    presetToday: 'Today',
    preset7d: 'Last 7 days',
    preset30d: 'Last 30 days',
    presetCustom: 'Custom range',
    resetFilters: 'Reset filters',
    totalFound: '{count} trade signals',
    noSignals: 'No trade signals for the selected filters.',
    colChannel: 'Channel',
    colSignal: 'Signal',
    colTime: 'Time',
    signalsToday: 'Signals today',
    signalsLast7d: 'Last 7 days',
    signalsLast30d: 'Last 30 days',
    signalsTotal: 'All time',
    actionBuy: 'Buy',
    actionSell: 'Sell',
    actionClose: 'Close',
    actionCloseWorseEntries: 'Close worse entries',
    actionBreakeven: 'Move SL to break-even',
    actionModify: 'Update SL/TP',
    actionPartialProfit: 'Take partial profit',
    actionPartialBreakeven: 'Partial profit + break-even',
    onSymbol: 'on {symbol}',
    entryAt: 'Entry {price}',
    slAt: 'SL {price}',
    tpAt: 'TP {prices}',
    statusOpen: 'Open',
    statusClosed: 'Closed',
    editSignal: 'Edit SL/TP',
    updateLiveTrades: 'Update Live Trades',
    saving: 'Sending Order',
    originalSignal: 'Channel signal',
    overrideSignal: 'Current override',
    applySuccess: 'Override saved. Applied to {count} open leg(s).',
    applyBrokerSummary: 'Updated {updated} of {total} broker(s).',
    applyPartialBrokers: 'Override saved. Updated {updated} of {total} broker(s) — the rest are being reconciled.',
    applyFailed: 'Could not save override',
    invalidLevels: 'Enter a valid SL and/or at least one TP level.',
    closedReadOnly: 'Closed signals are read-only.',
    stopLoss: 'Stop loss',
    takeProfits: 'Take profit levels',
    addTp: 'Add TP',
    removeTp: 'Remove TP',
    closeModal: 'Close',
    colStatus: 'Status',
    updatesReceivedToday: 'Signals today',
    updatesReceivedLast7d: 'Last 7 days',
    updatesReceivedLast30d: 'Last 30 days',
    updatesReceivedTotal: 'All time',
    updatesReceivedThisWeek: 'This week',
    updatesReceivedThisMonth: 'This month',
  },
  trades: {
    title: 'Trades',
    subtitle: 'Live positions and recent closes from your linked broker accounts',
    filterOpen: 'Open',
    filterClosed: 'Closed',
    filterAll: 'All',
    refresh: 'Refresh',
    emptyTitle: 'No trades to show',
    emptySubtitleConnect:
      'Connect a broker account in {page} to see live trades here.',
    emptySubtitleOpen: 'No open positions on any of your linked broker accounts.',
    emptySubtitleClosed: 'No recent closed orders in this MT session.',
    modalTitle: 'Trade details',
    close: 'Close',
    manage: 'Manage',
    tradeSummary: 'Trade',
    signalChannel: 'Signal channel',
    telegramMessage: 'Telegram message',
    parsedInstruction: 'Parsed instruction',
    signalTime: 'Signal received',
    noLinkedSignal: 'No linked TScopier signal for this trade.',
    imageSignal: '(image signal)',
    loadingSignal: 'Loading signal…',
    loadSignalError: 'Could not load signal details.',
    viewDetails: 'View details',
    statusOpen: 'Open',
    statusClosed: 'Closed',
    colBroker: 'Broker',
    colEntry: 'Entry',
    colSl: 'SL',
    colTp: 'TP',
    colLots: 'Lots',
    colPnl: 'PnL',
    colTime: 'Date & Time',
    instructionAction: 'Action',
    instructionSymbol: 'Symbol',
    instructionEntry: 'Entry',
    instructionEntryZone: 'Entry zone',
    instructionSl: 'Stop loss',
    instructionTp: 'Take profit',
    instructionLotSize: 'Lot size',
    instructionMessage: 'Instruction',
    report: 'Report',
    reportTitle: 'Report this trade',
    reportSubtitle: 'Tell us what looks wrong — our support team reviews every report.',
    reportCategory: 'What looks wrong?',
    catWrongEntry: 'Wrong entry price',
    catWrongSl: 'Wrong stop loss',
    catWrongTp: 'Wrong take profit',
    catWrongDirection: 'Wrong direction',
    catWrongLots: 'Wrong lot size',
    catNotExecuted: 'Trade was not executed',
    catOther: 'Other',
    reportReason: 'Describe the problem',
    reportReasonPlaceholder: 'e.g. entry was at 4235 but my trade filled at 4250…',
    reportSubmit: 'Submit report',
    reportSuccess: 'Report submitted — our support team will review it shortly.',
    reportError: 'Could not submit the report. Please try again.',
    reportsTitle: 'Reported trades',
    reportsSubtitle: 'Status of the trades you reported. Our support team reviews each report.',
    reportsEmpty: 'No reported trades yet. You can report a trade from the Trades page or ask the assistant.',
    reportsStatusOpen: 'Open',
    reportsStatusResolved: 'Resolved',
    brokerErrorTitle: 'Broker error',
    brokerErrorIntro: 'The broker rejected or failed one or more orders for this trade.',
    viewCopierLogs: 'View in Copier Logs',
    cancel: 'Cancel',
  },
  channelsPage: {
    title: 'Channels',
    subtitle: 'Manage which Telegram channels you\'re monitoring',
    addChannel: 'Add channel',
    addFormTitle: 'Add channel manually',
    channelName: 'Channel name',
    channelNamePlaceholder: 'e.g. Gold Signals Pro',
    usernameOptional: 'Username (optional)',
    usernamePlaceholder: '@channelname',
    channelIdOptional: 'Channel ID (optional)',
    channelIdPlaceholder: 'Telegram channel ID',
    channelIdHint: 'Leave blank to use username',
    nameRequired: 'Channel name is required',
    emptyTitle: 'No channels yet',
    emptySubtitle: 'Add a signal channel to start monitoring',
    statusPaused: 'Paused',
    overridesTitle: 'Per-channel overrides',
    lotSizeOverride: 'Lot size override',
    pipToleranceOverride: 'Pip tolerance override',
    useBrokerDefault: 'Use broker default',
  },
  popularChannelsPage: {
    title: 'Popular Channels',
    subtitle: 'Signal channels ranked by subscriber count across all TSCopier users',
    emptyTitle: 'No channels yet',
    emptySubtitle: 'Channels will appear here as users subscribe to them',
  },
  copierEnginePage: {
    connectTelegram: 'Connect Telegram',
    telegramNotConnectedTitle: 'Telegram not connected.',
    telegramNotConnectedBody: 'Connect Telegram here to load and manage your channel list.',
    tgConnectHeroTitle: 'Link your Telegram account',
    tgConnectHeroSubtitle: 'Sign in with your phone to browse signal channels and add them to the copier.',
    tgConnectPhoneTitle: 'Enter your phone number',
    tgConnectPhoneSubtitle: 'We’ll send a verification code to your Telegram app (not SMS).',
    tgConnectCodeTitle: 'Enter verification code',
    tgConnectCodeSubtitle: 'Open the Telegram app on your phone — the login code usually appears there, not as an SMS.',
    tgConnectStepPhone: 'Phone',
    tgConnectStepCode: 'Verify',
    tgConnectStepTwoFa: '2FA',
    tgConnectStepChannels: 'Channels',
    tgConnectTwoFaTitle: 'Two-step verification',
    tgConnectTwoFaSubtitle: 'Your account has 2FA enabled. Enter your Telegram cloud password to finish connecting.',
    backToVerificationCode: 'Back to verification code',
    tgConnectStepsAria: 'Telegram connection steps',
    tgConnectHowItWorks1: 'Enter the phone number linked to your Telegram account',
    tgConnectHowItWorks2: 'Enter the code sent in the Telegram app',
    tgConnectHowItWorks3: 'Choose signal channels to monitor and copy',
    tgConnectPhoneWarning:
      'TScopier does not access or read your messages. By connecting your Telegram, you only grant the app access to channels you\'re a member of.',
    tgConnectMethodTitle: 'Choose how to sign in',
    tgConnectMethodSubtitle: 'Use your phone number or scan a QR code with the Telegram app on your phone.',
    tgConnectMethodPhone: 'Phone number',
    tgConnectMethodQr: 'QR code',
    tgConnectStepQr: 'Scan QR',
    tgConnectQrTitle: 'Scan with Telegram',
    tgConnectQrSubtitle: 'Link this account by scanning the code from your phone.',
    tgConnectQrInstructions:
      'Open Telegram on your phone → Settings → Devices → Link Desktop Device → scan this QR code.',
    tgConnectQrWaiting: 'Waiting for scan…',
    tgConnectQrExpired: 'QR code expired — generating a new one…',
    tgConnectQrTwoFaSubtitle: 'Your account has 2FA enabled. Enter your Telegram cloud password to finish linking.',
    failedStartQr: 'Failed to start QR login',
    noPendingQr: 'QR login expired. Please start again.',
    noPendingPhoneAuth: 'Login session expired. Go back and request a new verification code.',
    useDifferentNumber: 'Use a different number',
    cancelConnect: 'Cancel',
    phoneLabel: 'Phone number',
    phonePlaceholder: '+1 234 567 8900',
    phoneHint: 'Include country code',
    sendCode: 'Send code',
    verificationCode: 'Verification code',
    verificationPlaceholder: '12345',
    sentTo: 'Sent to {phone}',
    twoFaPassword: '2FA password',
    twoFaPlaceholder: 'Your Telegram password',
    twoFaRequired: 'Enter your Telegram 2FA password.',
    verify: 'Verify',
    back: 'Back',
    networkError: 'Network error',
    failedSendCode: 'Failed to send code',
    verificationFailed: 'Verification failed',
    telegramAlreadyLinked:
      'This Telegram account is already linked to another TScopier account. Sign in to that account or contact support.',
    failedLoadTgChannels: 'Failed to load Telegram channels',
    telegramSessionExpired: 'Your Telegram session expired. Reconnect to resume — your configured channels are kept.',
    telegramConnectionBusy: 'Telegram is reconnecting after a backtest or another task. Wait 30 seconds, then press Refresh — or use Reconnect Telegram.',
    reconnectTelegram: 'Reconnect Telegram',
    channelSearchPlaceholder: 'Search channels…',
    noChannelSearchResults: 'No channels match your search',
    yourTelegramChannels: 'Your Telegram channels',
    telegramConnectedHint: 'Add channels below to start copying signals',
    connected: 'Connected',
    listenerLeaseLive: 'Copier engine live',
    listenerLeaseExpired: 'Copier engine offline',
    listenerLeaseExpiredHint:
      'Trades may not copy until the worker recovers. Wait a minute, press Refresh, or use Reconnect Telegram.',
    listenerLeaseUnknown: 'Checking copier engine…',
    expand: 'Expand',
    collapse: 'Collapse',
    disconnect: 'Disconnect',
    channelsFound: '{count} found',
    refreshAfterFix: 'Use Refresh after fixing Telegram connection or worker issues.',
    noTgChannelsTitle: 'No channels or groups found',
    noTgChannelsSubtitle: 'Make sure you are a member of the signal channels',
    members: '{count} members',
    added: 'Added',
    add: 'Add',
    configuredEmptyTitle: 'No channels configured',
    configuredEmptySubtitle: 'Add Telegram channels above to start the copier',
    configuredEmptyConnectHint: 'Complete Telegram setup above, then add channels to start the copier.',
    activeChannels: 'Active channels',
    configuredCount: '{count} configured',
    statusPaused: 'Paused',
    removeAria: 'Remove {label}',
    analyzing: 'Analyzing last 30 days… {percent}%',
    profilePending: 'Profile insights will appear after analysis completes.',
    profileAnalyzeFailed: 'Channel analysis failed. Check that Telegram is connected and the listener worker is running.',
    profileNoMessages: 'No trading messages found in the last 30 days.',
    profileType: 'Type: {value}',
    profileEntry: 'Entry: {value}',
    profileTp: 'TP: {value}',
    profileSl: 'SL: {value}',
    connectedBrokers: 'Connected brokers',
    connectToBroker: 'Connect to Broker',
    connectAllBrokers: 'Connect all brokers',
    connectAllBrokersAria: 'Link {channel} to all active broker accounts',
    addBrokerConnectionAria: 'Connect {channel} to another broker',
    removeBrokerConnectionAria: 'Disconnect {broker} from {channel}',
    noBrokersYet: 'No broker accounts yet',
    connectBrokerInConfig: 'Add a broker in Account Configuration first',
    channelTelegramId: 'Telegram ID: {id}',
    invalidChannelIdentity:
      'Invalid Telegram ID — remove this channel and re-add it from the Telegram picker above.',
    manualIdentityRequired: 'Enter a numeric Telegram channel ID or @username.',
    invalidChannelIdFormat: 'Telegram channel ID must be numeric (e.g. -1001234567890).',
    invalidUsernameFormat: 'Telegram @username must be 5–32 characters (letters, numbers, underscore only).',
    keywordsTitle: 'Channel Keywords',
    keywordsClose: 'Close',
    keywordsSave: 'Save Keywords',
    signalKeywordSection: 'Signal Keyword',
    updateKeywordSection: 'Update Keyword',
    additionalKeywordSection: 'Additional Keyword',
    signalKeywords: {
      entryPoint: 'Entry Point',
      buy: 'BUY',
      sell: 'SELL',
      sl: 'SL',
      tp: 'TP',
      marketOrder: 'Market Order',
    },
    updateKeywords: {
      closeTp1: 'Close TP1',
      closeTp2: 'Close TP2',
      closeTp3: 'Close TP3',
      closeTp4: 'Close TP4',
      closeFull: 'Close Full',
      closeHalf: 'Close Half',
      closePartial: 'Close Partial',
      breakEven: 'Break Even',
      setTp1: 'Set TP1',
      setTp2: 'Set TP2',
      setTp3: 'Set TP3',
      setTp4: 'Set TP4',
      setTp5: 'Set TP5',
      setTp: 'Set TP',
      adjustTp: 'Adjust TP',
      setSl: 'Set SL',
      adjustSl: 'Adjust SL',
      delete: 'Delete',
    },
    additionalKeywords: {
      layer: 'Layer',
      closeAll: 'Close All',
      deleteAll: 'Delete All',
      ignoreKeyword: 'Ignore Keyword',
      skipKeyword: 'Skip Keyword',
      removeSl: 'Remove SL',
      delayMsec: 'Delay in Msec',
      preferEntry: 'Prefer Entry',
      slInPips: 'SL In Pips',
      tpInPips: 'TP In Pips',
      delimiters: 'Delimiters',
      allOrder: 'ALL Order',
      readForwarded: 'Read Forwarded',
      readImage: 'Read Image',
      firstPrice: 'First Price',
      lastPrice: 'Last Price',
    },
  },
  backtest: {
    title: 'Channel Signal Backtest',
    subtitle:
      'Replay your Telegram signal history with live market data. Test breakeven rules, TP ladders, and portfolio sizing before risking live capital.',
    channels: 'Channels',
    symbols: 'Symbols',
    from: 'From',
    to: 'To',
    timeframe: 'Timeframe',
    execution: 'Execution',
    executionTick: 'Tick quotes (forex)',
    executionBars: 'OHLC bars',
    strategy: 'Strategy',
    breakevenAfterTp: 'Breakeven after TP',
    breakevenDisabled: 'Disabled',
    intrabarPriority: 'Intrabar priority',
    intrabarSlFirst: 'SL first (conservative)',
    intrabarTpFirst: 'TP first (optimistic)',
    account: 'Account',
    startingBalance: 'Starting balance ($)',
    sizing: 'Sizing',
    sizingFixed: 'Fixed lot',
    sizingRisk: 'Risk % per trade',
    lotSize: 'Lot size',
    riskPercent: 'Risk % of equity',
    runBacktest: 'Run backtest',
    recentRuns: 'Recent runs',
    history: 'History',
    signalBreakdown: 'Signal breakdown',
    noActiveChannels: 'No active Telegram channels',
    configureHint: 'Choose channel and date range, then pull signals from Telegram.',
    signalChannel: 'Signal channel',
    pullingSignals: 'Pulling signals…',
    pullProfileSignals: 'Pull & profile signals',
    selectChannelError: 'Select a signal channel',
    profileFirstError: 'Profile signals first',
    selectSymbolError: 'Select a symbol',
    profileImported: 'Imported {imported} signal(s) from {scanned} messages.',
    profileCandidates: '{candidates} candidate(s) found; none stored as tradeable.',
    profileNoTradeable: 'No tradeable signals in range ({scanned} messages scanned).',
    profileSyncFailed: 'Could not import signals from Telegram. Check that your listener worker is running and BACKTEST_WORKER_URL is configured.',
    noSymbolsInRange: 'No tradeable symbols in this date range. Widen the range, pick another channel, or pull signals again.',
    readyTitle: 'Ready to backtest',
    readyMeta: '{channel} · {count} signals · {from} → {to}',
    back: 'Back',
    symbolToBacktest: 'Symbol to backtest',
    runningDefault: 'Running backtest on market data…',
    resultReady: 'Your backtest finished. Review pip results and per-signal breakdown.',
    seeBacktestResult: 'See Backtest Result',
    resultsTitle: 'Backtest results',
    resultsSubtitle: '{symbol} · {channel} · tap a signal for details',
    newRun: 'New run',
    exportCsv: 'Export CSV',
    totalPips: 'Total pips',
    winRate: 'Win rate',
    winLoss: 'W / L',
    signalsLabel: 'Signals',
    oneSignal: '1 signal',
    nSignals: '{count} signals',
    channelFallback: 'Channel',
    channelMore: '{name} +{count}',
    historyModalTitle: 'Backtest history',
    historyModalSubtitle: 'Past runs for your account',
    historyEmpty: 'No backtest runs yet. Run a backtest to see it here.',
    close: 'Close',
    statusCompleted: 'Completed',
    statusFailed: 'Failed',
    statusRunning: 'Running',
    statusCancelled: 'Cancelled',
    noResults: 'No signal results for this run.',
    buy: 'Buy',
    sell: 'Sell',
    resultsPerPage: 'Results per page',
    showing: 'Showing',
    resultModalTitle: 'Backtest result',
    editSignal: 'Edit signal',
    direction: 'Direction',
    entry: 'Entry price',
    stopLoss: 'Stop loss',
    takeProfits: 'Take profits',
    addTp: 'Add TP',
    removeTp: 'Remove TP',
    rerunCheck: 'Re-run check',
    rerunning: 'Re-checking on market data…',
    deleteResult: 'Delete result',
    deleteConfirm: 'Remove this signal from the backtest run? Totals will be updated.',
    deleteFailed: 'Could not delete this result.',
    rerunFailed: 'Could not re-run the check.',
    invalidLevels: 'Enter a valid entry price and at least one stop loss or take profit.',
    unsavedHint: 'Save changes with Re-run check to refresh outcome and totals.',
    pips: 'Pips',
    riskReward: 'R:R',
    duration: 'Duration',
    eventTimeline: 'Event timeline',
    noEvents: 'No timed events recorded for this signal.',
    outcomes: {
      allTpHit: 'All TPs',
      tpThenBe: 'TP → BE',
      partial: 'Partial',
      tp1ThenSl: 'TP1 → SL',
      slHit: 'SL Hit',
      breakeven: 'BE',
      noData: 'No data',
      skipped: 'Skipped',
      open: 'Open',
    },
    banners: {
      allTpHit: 'All TPs Hit',
      slHit: 'SL Hit',
      breakeven: 'Breakeven',
      tpThenBe: 'TP → Breakeven',
      partialHit: 'Partial Hit',
      noMarketData: 'No Market Data',
      skipped: 'Skipped',
      open: 'Open',
    },
    events: {
      tpHit: 'TP{n} Hit',
      slHit: 'SL Hit',
      breakeven: 'Breakeven',
    },
    priceLevels: {
      entry: 'Entry',
      sl: 'SL',
      be: 'BE',
      tp: 'TP{n}',
    },
    replayTitle: 'Price replay',
    replayLoading: 'Loading price replay…',
    replayError: 'Could not load price replay.',
    replayPlay: 'Play',
    replayPause: 'Pause',
    replayScrub: 'Scrub replay',
    replaySourceTicks: 'Tick data',
    replaySourceBars: 'M1 bars (ticks unavailable)',
    replayExit: 'Exit',
    errors: {
      rateLimit:
        'Market data unavailable — ensure your broker is connected in Brokers and try again.',
    },
  },
  brokersPage: {
    title: 'MT5 broker terminals',
    subtitle: 'Connect and test cloud MT5 terminals side-by-side with your legacy mt4api.dev brokers.',
    mt5OnlyNote: 'Cloud MT5 linking supports MT5 only. Legacy compare may be MT4 — quote and summary still work; demo trade compare is skipped for MT4.',
    apiKeyRotationNote: 'After rotating your broker API key, allow ~5 minutes for terminals to pick up the new key. Brief 5xx responses during restart are normal.',
    setupNote: 'Link MT5 accounts with login, password, and server — only the FXSOCKET_API_KEY Edge secret is required (POST /v1/accounts). Remove any old FXSOCKET_CONNECT_URL secret.',
    sectionAccounts: 'Linked MT5 accounts',
    sectionCompare: 'Side-by-side comparison',
    sectionRawLog: 'Raw API log',
    connectTitle: 'Connect MT5 account',
    labelField: 'Label',
    loginField: 'MT5 login',
    passwordField: 'Password',
    serverField: 'Broker server',
    connectButton: 'Connect',
    connecting: 'Spinning up terminal…',
    refresh: 'Refresh',
    delete: 'Remove',
    copyUuid: 'Copy UUID',
    uuidCopied: 'UUID copied',
    openSwagger: 'Open Swagger',
    lastSynced: 'Last synced {time}',
    noAccounts: 'No linked MT5 accounts yet. Connect a demo account to start testing.',
    compareFxAccount: 'Cloud MT5 account',
    compareLegacyBroker: 'Legacy broker (Configuration)',
    symbolField: 'Symbol for quote test',
    runDiagnostics: 'Run diagnostics',
    runDemoTrade: 'Run demo trade test',
    compareRunning: 'Running comparison…',
    compareTableOperation: 'Operation',
    compareTableFxMs: 'Cloud (ms)',
    compareTableLegacyMs: 'Legacy (ms)',
    compareTableDelta: 'Delta',
    compareTableStatus: 'Status',
    statusOk: 'OK',
    statusError: 'Error',
    demoTradeTitle: 'Confirm demo trade test',
    demoTradeBody: 'This sends a micro-lot market buy on both the cloud MT5 terminal and the selected legacy demo broker (MT5 only). Only proceed on demo accounts you control.',
    demoTradeConfirm: 'Send demo test trade',
    demoTradeCancel: 'Cancel',
    connectionStatus: {
      connecting: 'Connecting',
      connected: 'Connected',
      error: 'Error',
      disconnected: 'Disconnected',
    },
    opLabels: {
      account_summary: 'Account summary',
      quote: 'Quote',
      opened_orders: 'Opened orders',
      order_send: 'Demo order send',
    },
    loadError: 'Could not load linked MT5 accounts.',
    connectError: 'Connect failed.',
    deleteConfirm: 'Remove this MT5 account from TScopier?',
    selectFxAccount: 'Select cloud MT5 account',
    selectLegacyBroker: 'Select legacy broker',
    docsUrl: 'https://fxsocket.com/docs#request-builder',
    docsLinkLabel: 'Trading API (Request Builder)',
    v1DocsUrl: 'https://api.fxsocket.com/v1/docs#/',
    v1DocsLinkLabel: 'Account linking API (v1)',
    docsEndpointNote: 'Trading and data calls run server-side via Edge — the broker API has no CORS headers.',
    linkExistingTitle: 'Advanced: link an account already connected on the cloud platform',
    existingUuidField: 'Account UUID (from your cloud terminal dashboard)',
    linkExistingButton: 'Link account',
    connectModeCredentials: 'Connect with MT5 credentials',
    connectModeLinkUuid: 'Link existing UUID',
  },
  marketNews: {
    title: 'Forex Market News',
    subtitle:
      'Latest forex headlines and analysis from global sources — refreshed on demand while you plan your next trade.',
    symbol: 'Currency pair',
    symbolAll: 'All pairs',
    forexBadge: 'Forex',
    readArticle: 'Read article',
    lastUpdated: 'Updated {time}',
    loadError: 'Could not load market news.',
    emptyTitle: 'No forex news right now',
    emptySubtitle: 'Try refreshing in a moment.',
    emptyFilterTitle: 'No news for this pair',
    emptyFilterSubtitle: 'No recent headlines for {pair}. Try another pair or refresh.',
    moreHeadlines: 'More headlines',
    dataByFmp: 'Market data by Financial Modeling Prep',
  },
  economicCalendar: {
    title: 'Economic Calendar',
    subtitle:
      'Upcoming macro releases with impact levels, forecasts, and actuals — filter by country and link related forex news.',
    from: 'From',
    to: 'To',
    country: 'Country / currency',
    countryAll: 'All countries',
    impact: 'Impact',
    impactAll: 'All impact',
    impactHigh: 'High',
    impactMedium: 'Medium',
    impactLow: 'Low',
    newsFilter: 'News filter — show related forex headlines',
    actual: 'Actual',
    forecast: 'Forecast',
    previous: 'Previous',
    loadError: 'Could not load economic calendar.',
    empty: 'No events in this range. Try widening the date window or changing filters.',
    lastUpdated: 'Updated {time}',
    relatedNews: 'Related news',
    relatedNewsEmpty: 'No matching headlines for the current filters.',
    readArticle: 'Read article',
    dataByFmp: 'Calendar data by Financial Modeling Prep',
  },
  performance: {
    title: 'Performance',
    subtitle:
      'Live stats from your linked MetaTrader accounts — closed trade history, balances, and period filters for charts and headline metrics.',
    noMtBroker:
      'Link an active MetaTrader account to load performance data. Paused or legacy accounts without a broker connection are not included.',
    noTradeHistory:
      'No closed trade history was returned from the broker. Check the connection on the Dashboard or try Refresh.',
    period7d: '7 days',
    period30d: '30 days',
    period90d: '90 days',
    periodAll: 'All time',
    lastUpdated: 'Updated {time}',
    realizedPnl: 'Realized P/L',
    closedTrades: '{count} closed trades',
    winRate: 'Win rate',
    winLoss: '{won} won · {lost} lost',
    profitFactor: 'Profit factor',
    avgRoi: 'Avg ROI',
    accountsTracked: '{count} accounts with baseline',
    noBaseline: 'Set a performance baseline per account',
    maxDrawdown: 'Max drawdown',
    outcomeTitle: 'Daily outcomes · last {days} days',
    outcomeTitleAll: 'Daily outcomes · all time',
    outcomeSubtitle: 'Closed-trade profit and loss by calendar day',
    outcomeEmpty: 'No closed trades in this period',
    accountsTitle: 'Account breakdown',
    accountsSubtitle: 'Live equity and lifetime stats per linked account',
    viewTrades: 'View trades',
    accountsEmpty: 'Connect a broker account to see performance metrics.',
    colAccount: 'Account',
    colBroker: 'Broker',
    colEquity: 'Equity',
    colRoi: 'ROI',
    colWinRate: 'Win rate',
    colMaxDrawdown: 'Max DD',
    configure: 'Configure',
    baselineNote:
      'ROI and drawdown use each account’s performance baseline from configuration. Period filters apply to trade stats and the outcome chart only.',
    analysisTitle: 'Deep dive',
    analysisSubtitle: 'Highlights and breakdowns for the selected period — closed trades, equity curve, and signal attribution.',
    bestDay: 'Best day',
    worstDay: 'Worst day',
    highestEquity: 'Highest equity',
    lowestEquity: 'Lowest equity',
    bestTrade: 'Best trade',
    worstTrade: 'Worst trade',
    profitByChannelTitle: 'Profit by signal channel',
    profitByChannelSubtitle: 'Realized P/L from closed trades linked to Telegram channels (ticket or copier comment).',
    symbolDistributionTitle: 'Symbol distribution',
    symbolDistributionSubtitle: 'Number of closed trades per symbol in this period.',
    distributionEmpty: 'No closed trades in this period for this breakdown.',
    tradesCount: 'Trades',
    unlinkedChannel: 'Unlinked / manual',
    otherSymbols: 'Other symbols',
    onDate: 'On',
  },
  settings: {
    title: 'Settings',
    subtitle: 'Manage your profile, preferences, and account security.',
    loadError: 'Could not load settings.',
    saveError: 'Could not save changes.',
    saved: 'Changes saved.',
    emailHint: 'Email is managed by your sign-in provider and cannot be changed here.',
    passwordHint: 'At least 8 characters.',
    passwordTooShort: 'Password must be at least 8 characters.',
    passwordMismatch: 'Passwords do not match.',
    passwordUpdated: 'Password updated successfully.',
    passwordError: 'Could not update password.',
    sections: {
      personal: 'Personal information',
      general: 'General settings',
      security: 'Security',
    },
    personal: {
      title: 'Personal information',
      description: 'Your contact details and how you appear in TScopier.',
    },
    general: {
      title: 'General settings',
      description: 'Defaults used for balances, reports, and timestamps.',
    },
    security: {
      title: 'Security',
      description: 'Update your password to keep your account secure.',
      updatePassword: 'Update password',
    },
    fields: {
      firstName: 'First name',
      lastName: 'Last name',
      username: 'Username',
      email: 'Email',
      country: 'Country',
      city: 'City',
      mobile: 'Mobile number',
      address: 'Address',
      baseCurrency: 'Base currency',
      timezone: 'Timezone',
      newPassword: 'New password',
      confirmPassword: 'Confirm password',
      notificationSound: 'Trade notification sound',
      notificationSoundDescription: 'Play a sound when a trade is executed, modified, or closed.',
      notificationEmail: 'Email notifications',
      notificationEmailDescription: 'Send an email when a signal needs your review.',
    },
    placeholders: {
      address: 'Street, building, postal code…',
      selectCountry: 'Select country',
      selectTimezone: 'Select timezone',
      selectCurrency: 'Select currency',
      searchCountry: 'Search countries…',
      searchTimezone: 'Search timezones…',
      searchCurrency: 'Search currencies…',
      noMatches: 'No matches',
    },
  },
  affiliate: {
    title: 'Affiliate Program',
    subtitle: 'Earn 10% recurring commission for life on every paying referral.',
    refresh: 'Refresh',
    totalEarned: 'Total earned',
    pendingPayout: 'Pending payout',
    paidOut: 'Paid out',
    activeReferrals: 'Active referrals',
    yourReferralCode: 'Your referral code',
    editReferralCode: 'Edit referral code',
    cancelEditReferralCode: 'Cancel',
    copyCode: 'Copy code',
    copied: 'Copied',
    yourReferralLink: 'Your referral link',
    copyLink: 'Copy link',
    payoutWalletAddress: 'USDT TRC20 wallet address',
    payoutWalletHint: 'Payout method is USDT TRC20 only. Enter your wallet address.',
    editWalletAddress: 'Edit wallet address',
    cancelEditWalletAddress: 'Cancel',
    saveWalletAddress: 'Save wallet address',
    walletAddressSaveFailed: 'Could not save wallet address.',
    customCodeLabel: 'Custom referral code',
    customCodePlaceholder: 'your-name-123',
    customCodeHint: 'You can use letters, numbers, or symbols (no spaces), 3 to 32 characters.',
    saveCustomCode: 'Save code',
    customCodeInvalid: 'Referral code must be 3-32 characters and cannot contain spaces.',
    customCodeSaveFailed: 'Could not save referral code.',
    referrals: 'Referrals',
    commissions: 'Commissions',
    noReferrals: 'No referrals yet.',
    noCommissions: 'No commissions yet.',
    source: 'Source',
    date: 'Date',
    invoice: 'Invoice',
    gross: 'Gross',
    commission: 'Commission',
    status: 'Status',
    howItWorks: 'How it works',
    stepShare: 'Share your referral link or code.',
    stepSubscribe: 'When someone signs up and subscribes, attribution is locked to your account.',
    stepEarn: 'You earn 10% commission on every paid renewal for life.',
    policyTitle: 'Affiliate Program Policy',
    policyLifetime: 'Commission is 10% for the lifetime of each referred paying customer.',
    policyFirstTouch: 'First valid referral attribution is permanent and cannot be changed.',
    policySelfReferral: 'Self-referrals are not eligible for commission.',
    policyRefunds: 'Refunded, disputed, or voided invoices are reversed from commission balances.',
    policyMinimumPayout: 'Manual payout batches are issued monthly with a minimum $50 threshold.',
  },
  pages: {
    accountConfiguration: {
      title: 'Brokers',
      description: 'Connect MetaTrader accounts and tune how each one copies signals.',
    },
    brokers: {
      title: 'Brokers',
      description: 'MT5 sandbox — connect, diagnose, and compare against legacy mt4api.dev.',
    },
    contactSupport: {
      title: 'Contact Support',
      description: 'Get help from the TScopier team with your account, copier setup, or technical issues.',
    },
    riskDisclaimer: {
      title: 'Risk disclaimer',
      description: 'Important risks of copy trading, signal providers, and automated execution.',
    },
    termsOfService: {
      title: 'Terms of Service',
      description: 'Terms governing your use of TScopier.',
    },
    privacyPolicy: {
      title: 'Privacy Policy',
      description: 'How we collect, use, and protect your information.',
    },
    cookiePolicy: {
      title: 'Cookie Policy',
      description: 'How TScopier uses cookies and similar technologies.',
    },
    featureRequest: {
      title: 'Feature Request',
      description: 'Suggest improvements and vote on what we should build next.',
    },
    partnerWithUs: {
      title: 'Partner with us',
      description: 'Explore partnership opportunities for signal providers, educators, and brokers.',
    },
    affiliateProgram: {
      title: 'Affiliate Program',
      description: 'Invite traders and earn 10% lifetime commission on paid subscriptions.',
    },
    billing: {
      title: 'Billing',
      description: 'View invoices, payment methods, and billing history.',
    },
    subscriptions: {
      title: 'Subscriptions',
      description: 'Manage your plan, upgrades, and renewal settings.',
    },
    marketNews: {
      title: 'Market News',
      description: 'Stay up to date with the latest financial market news and analysis.',
    },
    economicCalendar: {
      title: 'Economic Calendar',
      description: 'Track upcoming economic events and their potential market impact.',
    },
    performance: {
      title: 'Performance',
      description: 'Deep-dive analytics on your trade history, win rate, and profitability.',
    },
    portfolio: {
      title: 'Portfolio',
      description: 'View and manage your overall trading portfolio across all connected accounts.',
    },
    analysisHub: {
      title: 'Analysis Hub',
      description: 'Advanced charting and AI-powered analysis tools for your trading signals.',
    },
    signalHistory: {
      title: 'Manage Signals',
      description: 'Trade signals from your connected Telegram channels — entries, closes, and SL/TP updates only.',
    },
    copierEngine: {
      title: 'Channels',
      description: 'Manage Telegram signal channels and copier settings.',
    },
    settings: {
      title: 'Settings',
      description: 'Profile, currency, timezone, and password..',
    },
  },
}
